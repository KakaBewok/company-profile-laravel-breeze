                ...
                <form method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <x-text-input id="name" class="block w-full mt-1" type="text" name="name" required
                            autofocus autocomplete="name" value="{{ $product->name }}" />
                        <x-input-error :messages="$errors->get('name')" class="mt-2" />
                    </div>

                    <div class="mt-4">
                        <x-input-label for="tagline" :value="__('tagline')" />
                        <x-text-input id="tagline" class="block w-full mt-1" type="text" name="tagline" required
                            autofocus autocomplete="tagline" value="{{ $product->tagline }}" />
